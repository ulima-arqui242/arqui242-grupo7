...
**Elección entre Gestión de Sobrecarga mediante Colas de Trabajo vs. Degradación Controlada para el manejo de saturación en la base de datos:**
...
1. Gestión de Sobrecarga mediante Colas de Trabajo
   ...
   **Ventajas:** Mantiene la operatividad del sistema al permitir la visualización continua de los servicios contratados.
   **Desventajas:** Las nuevas solicitudes pueden demorarse hasta 30 minutos en ser procesadas durante períodos de alta carga.
...
Dado que el tiempo de respuesta es crítico para la satisfacción del usuario, esta táctica proporciona una solución efectiva para manejar la saturación de la base de datos sin interrumpir la funcionalidad de visualización. Además, aunque puede haber demoras en el procesamiento de nuevas solicitudes, la continuidad operativa y la experiencia del usuario se ven preservadas, lo cual es crucial durante las horas pico.
...
| Código de Escenario | Estímulo | Fuente del Estímulo | Artefacto | Entorno | Respuesta | Medida de Respuesta |
|---------------------|----------|---------------------|-----------|---------|-----------|---------------------|
| ESC-01              | El Usuario solicitante de servicio no tiene un número de dirección de referencia | Usuario no tiene un numero de dirección valida registrada | Módulo de Contacto y Seguimiento de Servicio | Operación en horario de alta demanda (picos de uso) | Suspender la capacidad de registrar nuevas interacciones o actualizaciones de estado, pero permitir la visualización de servicios ya contratados | Plazo de 30 minutos para aumentar la capacidad de almacenamiento de la base de datos |
